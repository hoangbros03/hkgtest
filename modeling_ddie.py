...
BERT_INPUTS_DOCSTRING = r"""
    Args:
        ...
        encoder_hidden_states  (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, sequence_length, hidden_size)`, `optional`, defaults to :obj:`None`):
            Sequence of hidden-states at the output of the last layer of the enchape :obj:`(batch_size, sequence_length)`, `optional`, defaults to :obj:`None`):
...
class BertForPreTraining(BertPreTrainedModel):
    ...
    Returns:
        :obj:`tuple(torch.FloatTensor)` comprising various elements depending on the configuration (:class:`~transformers.BertConfig`) and inputs:
        loss (`optional`, returned when ``maskedf the model at the output of each layer plus the initial embedding outputs.
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def forward(
        ...
        lm_labels (:obj:`torch.LongTensor` of shape :obj:`(batch_size, sequence_length)`, `optional`, defaults to :obj:`None`):
            Labels for computing the left-to-right language modeling loss (next woptional`, returned when ``config.output_attentions=True``):
            Tuple of :obj:`torch.FloatTensor` (one for each layer) of shape
            :obj:`(batch_size, num_heads, sequence_length, sequence_length)`.
            Attentions weights after the attention softmax, used to compute the weighted average in the self-attention
            heads.
...
class MyModel(BertPreTrainedModel):
    ...
    def init_modules(self, model_args, kg_emb_file):
        if not model_args.baseline:
            ...
            self.bert.embeddings.word_embeddings = nn.Embedding.from_pretrained(word_kg_emb_table,
                                                                freeze=model_args.freeze_embeddings)

        self.dropout = nn.Dropout(model_args.dropout_ratio)

        self.use_cls_rep = model_args.use_cls_rep
        self.use_mention_rep = model_args.use_mention_rep
        self.use_kg_rep = model_args.use_kg_rep
        self.combination_method = model_args.combination_method

        self.mention_linear = nn.Linear(self.config.hidden_size*2, self.num_labels)
        self.kg_linear = nn.Linear(self.config.hidden_size*2, self.num_labels)
        ...
        if self.use_kg_rep: classifier_input_size += 2*self.config.hidden_size
    ...
    def forward(
        ...
        e1_kg_hidden_states = hidden_states[torch.arange(bsz), entity_position_ids[:, 2]]
        e2_kg_hidden_states = hidden_states[torch.arange(bsz), entity_position_ids[:, 3]]
        ...
        if self.combination_method == 'cat':
            ...
            if self.use_mention_rep:
                classifier_input += [e1_hidden_states, e2_hidden_states]
            if self.use_kg_rep:
                classifier_input += [e1_kg_hidden_states, e2_kg_hidden_states]
